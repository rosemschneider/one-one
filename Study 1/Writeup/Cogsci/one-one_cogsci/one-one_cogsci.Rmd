...
# Introduction
Human numerical abilities are built upon a foundation of core cognitive mechanisms shared with nonhuman animals; humans, however, enjoy a concept of number that is both \emph{exact} and \emph{unbounded}, and far exceeds the limits of what these foundational systems afford [@carey2019]. Humans are distinct from other animals in another critical sense, however, in that they regularly use symbols to externalize these exact number representations. This deep relationship between the uniquely human capacities of symbolic expression and exact number representation has prompted an enduring questions about whether representations of exactness depend upon knowledge of a symbolic number system. 

There is wide agreement that humans possess two mechanisms capable of supporting nonsymbolic numerical representations, even without exposure to number language: the Parallel Individuation (PI) system, which can track and represent small sets (up to 3 or 4), and the Approximate Number System (ANS), which offers nonprecise representations of large quantities [@feigenson2004]. These mechanisms are available early in life, and perhaps even in infancy [@wynn1992addition; @izard2009]. While both systems are refined over development, even together in their mature form they are incapable of supporting large exact number representations: although the PI system furnishes exact representations, these are limited to numerosities of 3 or 4, and while the ANS supports large number representations, they are imprecise. In particular, a key failing of the ANS in capturing integer properties is that it operates as a function of the ratio between two quantities (i.e., Weber's law). Thus, the ANS cannot detect differences in quantity when ratios are sufficiently small (e.g., 9:10).
...
There is some evidence that, in the absence of exact number language, human numerical representations may be constrained by the limits of these two systems; that is, exact number concepts may not be innate, but may depend on the acquisition of symbolic number systems [@nunez2017]. For example, @gordon2004 found that the Pirahã, an indigenous Amazonian culture with no exact number language [@frank2008], were unable to create exact matches for quantities beyond their PI range. In this work, @gordon2004 showed participants rows with varying numbers of items, and asked them to generate a match with their own set of items, with the simplest version requiring the creation of a matching set directly below the experimenter's. If a participant understands that number \emph{can} be represented exactly, then they should use one-to-one correspondence to establish a matching set for all numerosities outside the PI range. That is, they should understand Hume’s Principle -- that two sets are equinumerous if and only if both sets can be put into one-to-one correspondence [@hume1739]. On the other hand, if no such knowledge exists, then the participant should use approximation, rather than one-to-one correspondence, for quantities within the ANS range. 

Using this diagnostic of exact number, @gordon2004, and later @everett2012, found that the Pirahã were able to establish equality for numbers only within the PI range on the set-matching task, and approximated for larger quantities, even when one-to-one correspondence could be easily established. While these results seem to suggest the concept of exact equality seems tied to symbolic number language, @frank2008 showed that the Pirahã \emph{could} in fact deploy one-to-one correspondence to establish exact quality for quantities beyond the PI range, although they were significantly less likely to do so when this correspondence was more difficult to establish (e.g., if the experimenter's set was hidden after a brief presentation). These discrepant findings, along with a number of outside (with the possibility that the Pirahã are capable of representing number exactly, but may fail to do so because of a cultural de-emphasis on exactness [@laurence2007]) (leave the status of exact number open question)
...
While the status of exact number concepts in innumerate cultures remains unclear, their origin has also been explored in young children acquiring symbolic number. Although most US children are exposed to number language from birth, they do not start to acquire meanings for a subset of these number words (such as \emph{one}, \emph{two}, and \emph{three}) until around 2.5 years of age [@wynn1990]. These "subset knowers" have acquired the meanings of some number words in isolation; for example, a "two-knower" may be able to generate sets corresponding to "one" and "two," but is unable to generate sets of "three," nor relate known number words to the count list [@wynn1992]. Around 4 years of age, however, children acquire some form of the “Cardinal Principle” (CP, @gelman1978), at which point they understand how to use the count routine to generate sets for number words [@wynn1990; @wynn1992], and seem to acquire a qualitatively different understanding of number relative to subset knowers [@sarnecka2008]. The developmental trajectory associated with children’s number acquisition is remarkably consistent [@mollicainprep], and can be reliably assessed in the lab. Thus, young children on the cusp of mastering the meanings of number words offer a compelling case study in which to explore the origin of exact number concepts and their relationship to symbolic number. 

Previous investigations of subset knowers' exact number knowledge have produced mixed findings, however. For example, @starkey1992 found that even children who had not yet begun to recite the count list were able to track whether equality between a set of eggs placed in egg cups through an occlusion event, although only for quantities up to 3. Additionally, @izard2014 found that while subset knowers could track equality between a set of puppets placed on branches of a toy tree, they failed to do so if the perceptual identity of the items within the set changed, suggesting they were tracking perceptual identity and not numerical equality. Similarly, @mix1999 and @mix1996 found that very young children's ability to match sets on the basis of one-to-one correspondence was significantly affected by the perceptual similarity of the sets. Finally, @negen2009 found that children's ability to nonsymbolically match sets (<5) is related to their knowledge of both number words and the CP. (@jaraettinger2017 found that subset knowers were able to use one-to-one correspondence to track changes to equality for large - something about language here). 

Together, the developmental work suggests that subset knowers may have some partial knowledge of exact equality, although this work is limited by the fact that numerosities queried fall predominantly within the PI range. Additionally, comparisons of nonsymbolic one-to-one knowledge between subset and CP-knowers are limited, leaving open the question of whether this knowledge is affected by learning the significance of the count routine. Thus, in an effort to both (speak to developmental literature, and to reconcile other work with innumerate cultures, we are replicating methods used in work with Piraha in large nonsymbolic test of exact number). 

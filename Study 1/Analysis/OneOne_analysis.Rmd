...
all.data %>%
  ...
  mutate(Task_item = as.numeric(as.character(Task_item)), 
         Response = as.numeric(as.character(Response)), 
         kl.group = ifelse((Knower_level == "0" | Knower_level == "1" | Knower_level == "2"), "0-2", 
                           ifelse((Knower_level == "4" | Knower_level == "5"), "4-5", "3")))%>%
  filter(Task_item > 2)%>%
  group_by(kl.group, Task_item, Response)%>%
  # summarise(n = n()) %>%
  ggplot(aes(x = Response, fill = kl.group)) +
  geom_vline(aes(xintercept = Task_item), linetype = "dashed") +
  geom_histogram() + 
